...
class KalimatNLGHandler:
    ...
    def _process_tokens_internal(self, tokens: List[str], token_types: List[str] = None, mode: str = "natural") -> Dict[str, Any]:
        ...
        # 7. STRATEGI HYBRID: Natural untuk multi-kata, minimal feedback untuk single-word
        if mode == "natural":
            total_tokens = len(prefix) + len([t for seg in norm_segments for t in seg]) + len(suffix)
            if total_tokens == 1:
                # SINGLE WORD: Gunakan base natural tanpa feedback panjang
                single_token = (prefix + [t for seg in norm_segments for t in seg] + suffix)[0]
                natural_text = humanize_label(single_token).capitalize() + "."
