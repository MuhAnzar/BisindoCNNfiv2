...
# (Old compute_per_class_ap removed - superseded by version at line 346+)
            ...
            try:
                if is_video:
                    ...
                    while cap.isOpened():
                        ...
                        # 1. Flip (Mirroring) - CRITICAL for selfie-trained models
                        frame = cv2.flip(frame, 1)
                        
                        # 2. Preprocess (CLAHE/Enhancement)
                        frame_enhanced = preprocess_for_detection(frame)
                ...
                if buffer_data:
                    ...
                    if len(Xn) > 0:
                         ...
                         # Live predict() passes raw uint8 from process_buffer_from_results
                         # directly to ensemble_predictions WITHOUT /255.0
                         data.append({
                             "Xn": Xn, "Mn": Mn,
                             "Xf": Xf, "Mf": Mf
                         })
...
def compute_per_class_ap(model, labels, model_type="abjad", num_samples=10, kata_handler=None):
    """
    Compute Average Precision per class using Real Data or Synthetic Fallback.
    
    Metodologi disesuaikan dengan logika aplikasi live:
    - Deteksi dianggap BENAR jika target ada di Top-5 prediksi
    - DAN confidence prediksi tertinggi >= 20% (CONFIDENCE_THRESHOLD)
    - Per-class AP = proporsi deteksi benar dari total sample per kelas
    - Juga menghitung Top-1, Top-5 strict accuracy untuk perbandingan
    """
    CONF_THRESHOLD = 20.0  # Match live app threshold (percentage)
    TOP_K = 5              # Match live app top_k
    ...
    correct_top1 = 0
    correct_top5 = 0
    correct_app_logic = 0  # Target in Top-5 AND top_conf >= 20%
    total_samples = 0
    
    # Per-class tracking
    class_top5_correct = {i: 0 for i in range(n_classes)}
    ...
    for class_idx, label in enumerate(labels):
        ...
        if real_inputs:
            # Predict using real images/videos
            for inp in real_inputs:
                prob = None
                if model_type == "abjad":
                    prob = model.predict(inp, verbose=0)[0]
                else:
                    # Kata: Ensemble Prediction
                    if isinstance(inp, dict):
                        if kata_handler:
                             prob = kata_handler.ensemble_predictions(
                                 inp["Xn"], inp["Mn"], 
                                 inp["Xf"], inp["Mf"]
                             )
                        else:
                             xn = np.expand_dims(inp["Xn"], axis=0)
                             mn = np.expand_dims(inp["Mn"], axis=0)
                             xf = np.expand_dims(inp["Xf"], axis=0)
                             mf = np.expand_dims(inp["Mf"], axis=0)
                             
                             p1 = model.predict([xn, mn], verbose=0)[0]
                             p2 = model.predict([xf, mf], verbose=0)[0]
                             prob = (p1 + p2) / 2.0
                ...
                if prob is not None:
                    ...
                    # Evaluate with app-matching logic
                    top_k_indices = prob.argsort()[-TOP_K:][::-1]
                    top_conf = float(prob[top_k_indices[0]]) * 100.0
                    
                    # Top-1
                    if np.argmax(prob) == class_idx:
                        correct_top1 += 1
                    
                    # Top-5
                    if class_idx in top_k_indices:
                        correct_top5 += 1
                        class_top5_correct[class_idx] += 1
                    
                    # App Logic: target in Top-5 AND top_conf >= threshold
                    if class_idx in top_k_indices and top_conf >= CONF_THRESHOLD:
                        correct_app_logic += 1
            ...
            for _ in range(num_samples):
                if model_type == "abjad":
                    ...
                    inputs = [np.random.rand(1, 20, 224, 224, 3).astype(np.float32), 
                              np.random.rand(1, 20).astype(np.float32)]
                    prob = model.predict(inputs, verbose=0)[0]
    ...
    app_acc = correct_app_logic / total_samples if total_samples > 0 else 0
    
    print(f"  [INFO] Top-1 Accuracy: {correct_top1}/{total_samples} = {top1_acc:.4f}")
    print(f"  [INFO] Top-5 Accuracy: {correct_top5}/{total_samples} = {top5_acc:.4f}")
    print(f"  [INFO] App-Logic Accuracy (Top-5 + Conf≥20%): {correct_app_logic}/{total_samples} = {app_acc:.4f}")
    
    # ===== PER-CLASS AP (using App-Logic: Top-5 hit rate) =====
    # AP per class = jumlah prediksi benar (target in Top-5) / total sample per kelas
    # Ini mencerminkan logika deteksi sebenarnya di aplikasi
    ap_per_class = {}
    for c in range(n_classes):
        if class_total[c] > 0:
            ap = class_top5_correct[c] / class_total[c]
            ap_per_class[labels[c]] = ap
    
    # Also compute sklearn-based AP for reference (using raw probs)
...
def main():
    ...
    abjad_ap, abjad_y_true, abjad_y_probs = compute_per_class_ap(abjad_model, abjad_labels, "abjad", num_samples=10)
    ...
    kata_ap, kata_y_true, kata_y_probs = compute_per_class_ap(kata_model, kata_labels, "kata", num_samples=10, kata_handler=kata_handler)
