...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const API_URL_HEALTH = "{{ config('services.python_api.url') }}/health";
        ...
        async function checkApiHealth() {
            try {
                console.log(`Checking API Health at: ${API_URL_HEALTH}`);
                const response = await fetch(API_URL_HEALTH);
                const data = await response.json();
                
                if (data.status === 'ok') {
                    console.log("✅ API Health Check Passed:", data);
                    // Optional: Show success toast only if needed, or just silent success
                    // showToast("Sistem AI Siap Digunakan!", 'success');
                } else {
                ...
                let msg = "Gagal terhubung ke Sistem AI.";
                if (error.message.includes('Failed to fetch')) {
                    msg += " Pastikan API Python (port 5000) sedang berjalan.";
                } else {
                ...
                const statusOverlay = document.getElementById('status-overlay');
                if (statusOverlay) {
                    statusOverlay.querySelector('h3').textContent = 'Sistem AI Terputus';
                    statusOverlay.querySelector('h3').className = 'text-red-500 font-bold text-xl mb-1';
                    statusOverlay.querySelector('p').textContent = 'Tidak dapat menghubungi server AI. Cek koneksi Anda.';
                }
            }
        }
        ...
        async function predictFullFrame() {
            ...
            try {
                ...
                console.error("Error predicting full frame:", err);
                if (!window.hasShownApiError) {
                    showToast("Gagal memproses gambar. Cek koneksi API.", 'error');
                    window.hasShownApiError = true; // Prevent spam
                    setTimeout(() => window.hasShownApiError = false, 5000);
                }
            } finally {
